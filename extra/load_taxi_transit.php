...
if (isset($_POST['username']) && isset($_POST['sessionId']) && isset($_POST['campus']) && isset($_POST['time']) && isset($_POST['transit_pos'])) {
	
    $username = $_POST['username'];
	$sessionId = $_POST['sessionId'];
    $campus = $_POST['campus'];
    $time = $_POST['time'];
    $transit_pos = $_POST['transit_pos'];
	
    if($util->isActiveSession($username,$sessionId)) {
    
      $transit = $func->loadTaxiTransit($username, $campus, $time, $transit_pos);
         
      if($transit != OPERATION_FAILED) {
          
        $response["data"] = array('user'=>$transit);
